...
const QueryAndVisualizeDataOutputSchema = z.object({
  ...
  })).optional().describe('Data for generating a chart. The x and y values can be of any type (string, number, or date).'),
...
The chart data should be an array of objects, each with an 'x' and a 'y' property.
Determine the most appropriate fields from the JSON for the x and y axes based on the user's question.
Also provide clear and descriptive labels for the x and y axes.
